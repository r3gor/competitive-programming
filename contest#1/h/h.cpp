...
int main(){
    ...
    while(n--){
        ...
        name+=" " + name2; // THIS FIX MY STUPID ERROR
        // cout<<"name:::"<<name<<endl;
        t+=nota;
 
        if (fabs(nota-min.second)<1e-9 && compare(name,min.first)<0){
            min= make_pair(name, nota);
        }
        if (nota < min.second){
            min= make_pair(name, nota);
        }

        if (fabs(nota - max.second)<1e-9 && compare(name,max.first)<0){
            max= make_pair(name, nota);
        }
        if (nota > max.second){
            max= make_pair(name, nota);
        }
